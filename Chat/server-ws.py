            ...
            if message.startswith('exchange'):
                ...
                try:
                    ...
                    if 1 <= num_days <= 10:
                        ...
                        for days_ago in range(num_days, 0, -1):
                            ...
                            if exchange_rates is not None:
                                ...
                                for rate in exchange_rates.get('exchangeRate', []):
                                    ...
                                    if currency in ["USD", "EUR"]:  # Filter EUR and USD currencies
                        ...
                        # Send the exchange rate information to all clients
                        await self.send_to_clients(f"Exchange rates for the last {num_days} days:\n{' | '.join(exchange_rate_data)}")
                        
                        # # Log the exchange command to a file
                        # await self.write_to_file(ws.name, num_days)
                    else:
                        await self.send_to_clients("Invalid number of days. Please provide a value between 1 and 10.")
                except ValueError:
                    await self.send_to_clients("Invalid input. Please provide a valid number of days.")
            else:
                await self.send_to_clients(f"{ws.name}: {message}")
    ...
    async def write_to_file(self, name, num_days):
        try:
            async with aiofiles.open("exchange_log.txt", "a") as f:
                await f.write(f"{name} executed the exchange command for the last {num_days} days\n")
